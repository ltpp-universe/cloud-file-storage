...
async fn resp_json(ctx: &Context, content_type: &str, code: usize, msg: &str, data: &str) {
...
async fn resp_bin(ctx: &Context, status_code: usize, content_type: &str, data: Vec<u8>) {
    let host: String = ctx
        .get_socket_addr()
        .await
        .unwrap_or(DEFAULT_SOCKET_ADDR)
...
async fn success_resp_json(ctx: &Context, content_type: &str, msg: &str, data: &str) {
    println_success!("success_resp_json", " => ", msg, " ", data);
    resp_json(ctx, content_type, 1, msg, data).await;
}

async fn error_resp_json(ctx: &Context, content_type: &str, msg: &str, data: &str) {
    println_error!("error_resp_json", " => ", msg, " ", data);
    resp_json(ctx, content_type, 0, msg, data).await;
}

async fn success_resp_bin(ctx: &Context, content_type: &str, data: Vec<u8>) {
    println_success!("success_resp_bin", " => ", content_type);
    resp_bin(ctx, 200, content_type, data).await;
}
...
async fn file_middleware(ctx: Context) {
    let controller_data: InnerContext = ctx.get().await;
    ...
    if file_path.contains("../") {
        return error_resp_json(&ctx, content_type, &format!("{} unsafe", FILE_NAME_KEY), "").await;
    }
    let body: Vec<u8> = async_read_from_file(&file_path).await.unwrap();
    success_resp_bin(&ctx, content_type, body).await;
}

async fn index_file(ctx: Context) {
    let _ = ctx.send_response(200, FILE).await;
}

async fn add_file(ctx: Context) {
    let controller_data: InnerContext = ctx.get().await;
    ...
    let content_type: &str = FileExtension::parse(&extension_name).get_content_type();
    if content_type.is_empty() {
        return error_resp_json(&ctx, APPLICATION_JSON, "file type not supported", "").await;
    }
    let file_data: &Vec<u8> = req.get_body();
    let file_data_len: usize = file_data.len();
    if file_data_len == 0 {
        return error_resp_json(&ctx, APPLICATION_JSON, "file can not empty", "").await;
    }
    ...
    if metadata(&full_path).await.is_ok() {
        return error_resp_json(&ctx, APPLICATION_JSON, "file already exist", "").await;
    }
    let write_res: Result<(), std::io::Error> = async_write_to_file(&full_path, file_data).await;
    if let Err(err) = write_res {
        return error_resp_json(&ctx, APPLICATION_JSON, &format!("{:?}", err), "").await;
    }
    ...
    success_resp_json(&ctx, APPLICATION_JSON, "ok", &encode_full_url).await;
}
...
async fn main() {
    ...
    server.route("/", index_file).await;
    server.route("/add_file", add_file).await;
